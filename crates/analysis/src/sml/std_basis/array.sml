...
created by the same call to a primitive array constructor such as array, fromList, etc.; otherwise
they are not equal. This also holds for arrays of zero length. Thus, the type ty array admits
equality even if ty does not.
!*)
signature ARRAY = sig
  eqtype 'a array
  ...
  val maxLen : int
  (*!
  array (n, init) creates a new array of length n; each element is initialized to the value init. If
  n < 0 or maxLen < n, then the Size exception is raised.
  !*)
  val array : int * 'a -> 'a array
  (*!
  fromList l creates a new array from l. The length of the array is length l and the i(th) element
  of the array is the i(th) element of the the list. If the length of the list is greater than
  ...
  (*!
  update (arr, i, x) sets the i(th) element of the array arr to x. If i < 0 or |arr| <= i, then the
  Subscript exception is raised.
  !*)
  val update : 'a array * int * 'a -> unit
  (*!
  vector arr generates a vector from arr. Specifically, the result is equivalent to Vector.tabulate
  (length arr, fn i => sub (arr, i))
  !*)
  val vector : 'a array -> 'a vector
  (*!
  These functions copy the entire array or vector src into the array dst, with the i(th) element in
  src, for 0 <= i < |src|, being copied to position di + i in the destination array. If di < 0 or if
  |dst| < di+|src|, then the Subscript exception is raised. Implementation note: In copy, if dst and
  src are equal, we must have di = 0 to avoid an exception, and copy is then the identity.
  !*)
  ...
  replace each element with the result. The more general modifyi supplies f with the array index of
  the corresponding element. The expression modify f arr is equivalent to modifyi (f o #2) arr.
  !*)
  ...
  more general functions foldli and foldri supply f with the array index of the corresponding
  element. Refer to the MONO_ARRAY manual pages for reference implementations of the indexed
  versions. The expression foldl f init arr is equivalent to: foldli (fn (_, a, x) => f(a, x)) init
  arr The analogous equivalences hold for foldri and foldr.
  !*)
  ...
  otherwise, they return NONE. The more general version findi also supplies f with the array index
  of the element and, upon finding an entry satisfying the predicate, returns that index with the
  element.
  !*)
  ...
  It is equivalent to not(exists (not o f) arr)).
  !*)
  val all : ('a -> bool) -> 'a array -> bool
  (*!
  collate f (a1, a2) performs lexicographic comparison of the two arrays using the given ordering f
  on elements.
  !*)
