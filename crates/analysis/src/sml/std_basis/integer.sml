...
(*!
Instances of the INTEGER signature provide a type of signed integers of either a fixed or arbitrary
precision, and arithmetic and conversion operations. For fixed precision implementations, most
arithmetic operations raise the exception Overflow when their result is not representable.
!*)
signature INTEGER = sig
  eqtype int
  (*!
  These convert between integer values of types int and LargeInt.int. The latter raises Overflow if
  the value does not fit. Int<M>.fromLarge o Int<N>.toLarge converts an integer from type Int<N>.int
  to Int<M>.int.
  !*)
  ...
  (*!
  If SOME(n), this denotes the number n of significant bits in type int, including the sign bit. If
  it is NONE, int has arbitrary precision. The precision need not necessarily be a power of two.
  !*)
  val precision : Int.int option
  (*!
  The minimal (most negative) and the maximal (most positive) integers, respectively, representable
  by int. If a value is NONE, int can represent all negative (respectively, positive) integers,
  within the limits of the heap size. If precision is SOME(n), then we have minInt = -2(n-1) and
  maxInt = 2(n-1) - 1.
  !*)
  ...
  floor(((i / j))). It raises Overflow when the result is not representable, or Div when j = 0. Note
  that rounding is towards negative infinity, not zero.
  !*)
  val div : int * int -> int
  (*!
  i mod j returns the remainder of the division of i by j. It raises Div when j = 0. When defined,
  (i mod j) has the same sign as j, and (i div j) * j + (i mod j) = i
  !*)
  val mod : int * int -> int
  (*!
  ...
  (*!
  i rem j returns the remainder of the division of i by j. It raises Div when j = 0. (i rem j) has
  the same sign as i, and it holds that (i quot j) * j + (i rem j) = i This is the semantics of most
  hardware divide instructions, so rem may be faster than mod.
  !*)
  val rem : int * int -> int
  (*!
  compare (i, j) returns LESS, EQUAL, or GREATER when i is less than, equal to, or greater than j,
  respectively.
  !*)
  ...
  representable. This can happen, for example, when int is an n-bit 2's-complement integer type, and
  ~ is applied to -2 (n-1).
  !*)
  val ~ : int -> int
  (*!
  ...
  (*!
  sameSign (i, j) returns true if i and j have the same sign. It is equivalent to (sign i = sign j).
  !*)
  ...
  StringCvt.OCT [+~-]?[0-7]+ StringCvt.DEC [+~-]?[0-9]+ StringCvt.HEX [+~-]?(0x | 0X)?[0-9a-fA-F]+
  Note that strings such as "0xg" and "0x 123" are scanned as SOME(0), even using a hexadecimal
  radix. The second expression returns SOME(i) if an integer i in the format [+~-]?[0-9]+ can be
  parsed from a prefix of the string s, ignoring initial whitespace; NONE is returned otherwise. The
  function fromString raises Overflow when an integer can be parsed, but is too large to fit in type
  int. It is equivalent to the expression StringCvt.scanString (scan StringCvt.DEC).
  !*)
