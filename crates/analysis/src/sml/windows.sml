...
Microsoft Microsoft Windows operating system model. These functions include the ability to create
and communicate with separate processes, as well as to interact with the registry and file
...
signature WINDOWS (* OPTIONAL *) = sig
  ...
  contains its type (e.g., "FAT" or "NTFS"); the serialNumber field contains the serial number; and
  the maximumComponentLength field specifies the maximum length of any component of a pathname on
  this system.
  !*)
  val getVolumeInformation : string -> { volumeName : string, systemName : string, serialNumber : SysWord.word, maximumComponentLength : int }
  (*!
  findExecutable name returns the full executable name associated with name, or NONE if no such file
  exists.
  !*)
  val findExecutable : string -> string option
  (*!
  launchApplication (file, arg) runs the specified executable file passing it the argument arg. It
  raises OS.SysErr if file is not executable or if it cannot be run. Implementation note: This
  ...
  represented by the string arg, redirecting standard input and standard output to the null device.
  It then waits for the subprocess to terminate, and returns its exit status. This is similar to
  OS.Process.system but it can be used in cases where the latter does not work, and its return value
  provides more information about the exit status of the child process. Implementation note: This
  corresponds to the use of CreateProcess.
  !*)
  val simpleExecute : string * string -> OS.Process.status
  (*!
  The type of a process created by execute. The type parameters are witness types for the types of
  streams that can be returned.
  !*)
  type ('a,'b) proc
  (*!
  ...
  corresponds to the use of CreateProcess. Redirection of the standard streams can be handled using
  the hStdInput and hStdOutput fields in the STARTUPINFO parameter.
  !*)
  ...
  when it terminated. If reap is applied again to pr, it should immediately return the previous exit
  status. Implementation note: Typically, one cannot rely on the underlying operating system to
  provide the exit status of a terminated process after it has done so once. Thus, the exit status
  probably needs to be cached. Also note that reap should not return until the process being
  monitored has terminated. In particular, implementations should be careful not to return if the
  process has only been suspended.
  !*)
  val reap : ('a, 'b) proc -> OS.Process.status
  (*!
  The Status substructure defines the possible system-specific interpretations of OS.Process.status
  values.
  !*)
