...
signature OPTION = sig
  ...
  representing the result of partially defined functions. It can be viewed as a typed version of the
  C convention of returning a NULL pointer to indicate no value.
  !*)
  ...
  (*!
  mapPartial f opt maps NONE to NONE and SOME(v) to f(v). The expression mapPartial f is equivalent
  to join o (map f).
  !*)
  val mapPartial : ('a -> 'b option) -> 'a option -> 'b option
  (*!
  compose (f, g) a returns NONE if g(a) is NONE; otherwise, if g(a) is SOME(v), it returns SOME(f
  v). Thus, the compose function composes f with the partial function g to produce another partial
  function. The expression compose (f, g) is equivalent to (map f) o g.
  !*)
  ...
  another partial function. The expression composePartial (f, g) is equivalent to (mapPartial f) o
  g.
  !*)
