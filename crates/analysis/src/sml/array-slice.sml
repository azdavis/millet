(*!
The ArraySlice structure provides an abstraction of subarrays for polymorphic arrays. A slice value
can be viewed as a triple (a, i, n), where a is the underlying array, i is the starting index, and n
is the length of the subarray, with the constraint that 0 <= i <= i + n <= |a|. Slices provide a
convenient notation for specifying and operating on a contiguous subset of elements in an array.
!*)
signature ARRAY_SLICE = sig
  type 'a slice
  (*!
  length sl returns |sl|, the length (i.e., number of elements) of the slice. This is equivalent to
  #3 (base sl).
  !*)
  ...
  (*!
  slice (arr, i, sz) creates a slice based on the array arr starting at index i of the array. If sz
  is NONE, the slice includes all of the elements to the end of the array, i.e., arr[i..|arr|-1].
  This raises Subscript if i < 0 or |arr| < i. If sz is SOME(j), the slice has length j, that is, it
  corresponds to arr[i..i+j-1]. It raises Subscript if i < 0 or j < 0 or |arr| < i + j. Note that,
  if defined, slice returns an empty slice when i = |arr|.
  !*)
  ...
  defined, subslice returns an empty slice when i = |sl|.
  !*)
  val subslice : 'a slice * int * int option -> 'a slice
  (*!
  base sl returns a triple (arr, i, n) representing the concrete representation of the slice. arr is
  the underlying array, i is the starting index, and n is the length of the slice.
  !*)
  ...
  (*!
  These functions copy the given slice into the array dst, with the i(th) element of src, for 0 <= i
  ...
  the corresponding element in the slice. The expression modify f sl is equivalent to modifyi (f o
  #2) sl.
  !*)
  ...
  (*!
  These functions apply f to each element of the slice sl, in order of increasing indices, until a
  true value is returned. If this occurs, the functions return the element; otherwise, they return
  NONE. The more general version findi also supplies f with the index of the element in the slice
  ...
  (*!
  exists f sl applies f to each element x of the slice sl, in order of increasing indices, until f x
  evaluates to true; it returns true if such an x exists and false otherwise.
  !*)
