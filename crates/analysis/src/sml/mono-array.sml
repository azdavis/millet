...
signature MONO_ARRAY = sig
  ...
  type elem
  (*!
  The corresponding monomorphic vector type. We denote the length of a vector vec of type vector by
  |vec|.
  !*)
  type vector
  (*!
  ...
  used as the i(th) element of the array. If the length of the list is greater than maxLen, then the
  ...
  function f. The expression app f arr is equivalent to: appi (f o #2) arr
  !*)
  ...
  (*!
  These fold the function f over all the elements of an array, using the value init as the initial
  ...
  element. The indexed versions could be implemented as: fun foldli f init seq = let val len =
  length seq fun loop (i, b) = if i = len then b else loop(i+1,f(i,sub(seq,i),b)) in loop(0,init)
  end fun foldri f init seq = let val len = length seq fun loop (i, b) = if i = ~1 then b else
  loop(i-1,f(i,sub(seq,i),b)) in loop(len-1,init) end The expression foldl f init arr is equivalent
  to: foldli (fn (_, a, x) => f(a, x)) init arr The analogous equivalences hold for foldri and
  foldr.
  !*)
  ...
  return NONE. The more general version findi also supplies f with the array index of the element
