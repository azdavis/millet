...
impl MetaTyVar {
  /// Returns whether this `MetaTyVar` was generated by a [`MetaTyVarGen`] after that `MetaTyVarGen`
  /// generated the `marker`.
  pub(crate) fn generated_after(&self, marker: &MetaTyVarMarker) -> bool {
    self.0 >= marker.0
  }
}
...
impl MetaTyVarGen {
  ...
  pub(crate) fn mark(&self) -> MetaTyVarMarker {
    MetaTyVarMarker(self.0)
  }
}

/// A marker for [`MetaTyVar`]s.
pub(crate) struct MetaTyVarMarker(u32);
...
pub(crate) fn generalize(
  ...
  meta_vars(
    subst,
    &mut |x| {
      if x.generated_after(mv_marker) {
        meta.insert(x, None);
      }
    },
    &ty_scheme.ty,
