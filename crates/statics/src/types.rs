...
impl Sym {
  ...
  pub(crate) const WORD: Self = Self(1);
  pub(crate) const REAL: Self = Self(2);
  pub(crate) const CHAR: Self = Self(3);
  pub(crate) const STRING: Self = Self(4);

  pub(crate) const BOOL: Self = Self(5);
  pub(crate) const ORDER: Self = Self(6);

  pub(crate) const LIST: Self = Self(7);
  pub(crate) const REF: Self = Self(8);

  pub(crate) fn special(&self) -> Option<&'static str> {
    // you can match on `const`s, not just constructors. in this case, it's pretty nifty.
    let s = match *self {
      Sym::EXN => "exn",
      Sym::INT => "int",
      Sym::WORD => "word",
