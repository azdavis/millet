signature EITHER = sig
  ...
  (*!
  `map (f, g) e` applies either `f` or `g` to the value in `e` and re-wraps the result with whatever
  case `e` originally was.
  !*)
  ...
  (*!
  Uses either the left or right function to combine the value in the either with the starting value.
  !*)
  ...
  (*!
  Applies the function to a left either and re-wraps the result. If the either was right, returns it
  unchanged.
  !*)
  val mapLeft : ('a -> 'aa) -> ('a, 'b) either -> ('aa, 'b) either
  (*!
  Applies the function to a right either and re-wraps the result. If the either was left, returns it
  unchanged.
  !*)
