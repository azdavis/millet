signature FN = sig
   ...
   *
   * Put another way, `curry f` returns a function `f'` that is identical to `f`, except `f'` takes
   * its arguments "curried". That is:
   *
   ...
   *
   * Put another way, `uncurry f` returns a function `f'` that is identical to `f`, except `f'`
   * takes its arguments "uncurried". That is:
   *
