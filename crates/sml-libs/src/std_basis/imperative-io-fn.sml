(*!
 * The optional ImperativeIO functor can be used to implement (derive) an imperative-style stream
 * I/O facility in terms of a lazy functional stream I/O facility. In the imperative style, input
 * and output operations do not return a new stream each time but cause side-effects on their
 * arguments. Most functions can raise the Io exception for various reasons, including illegal or
 * inconsistent parameters, IO failures, and attempts to do I/O on closed output streams.
 *
