...
Following the concrete syntax provided by the list :: operator, the head of a list appears leftmost.
Thus, a traversal of a list from left to right starts with the head, then recurses on the tail. In
addition, as a sequence type, a list has an indexing of its elements, with the head having index 0,
the second element having index 1, etc.
!*)
signature LIST = sig
  ...
  (*!
  nth (l, i) returns the i(th) element of the list l, counting from 0. It raises Subscript if i < 0
  or i >= length l. We have nth(l,0) = hd l, ignoring exceptions.
  !*)
  val nth : 'a list * int -> 'a
  (*!
  take (l, i) returns the first i elements of the list l. It raises Subscript if i < 0 or i > length
  l. We have take(l, length l) = l.
  !*)
  ...
  with SOME stripped, where f was defined. f is not defined for an element of l if f applied to the
  element returns NONE. The above expression is equivalent to: ((map valOf) o (filter isSome) o (map
  f)) l
  !*)
  ...
  (*!
  partition f l applies f to each element x of l, from left to right, and returns a pair (pos, neg)
  where pos is the list of those x for which f x evaluated to true, and neg is the list of those for
  which f x evaluated to false. The elements of pos and neg retain the same relative order they
  possessed in l.
  !*)
  ...
  (*!
  exists f l applies f to each element x of the list l, from left to right, until f x evaluates to
  ...
  (*!
  tabulate (n, f) returns a list of length n equal to [f(0), f(1), ..., f(n-1)], created from left
  to right. It raises Size if n < 0.
  !*)
  val tabulate : int * (int -> 'a) -> 'a list
  (*!
  collate f (l1, l2) performs lexicographic comparison of the two lists using the given ordering f
