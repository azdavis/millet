 ...
 * index, and n is the length of the subvector, with the constraint that 0 <= i <= i + n <= |v|,
 * where |v| is the length of v. Slices provide a convenient notation for specifying and operating
 * on a contiguous subset of elements in a vector.
 *)
signature VECTOR_SLICE = sig
   ...
   * |arr| < i + j. Note that, if defined, slice returns an empty slice when i = |vec|.
   *)
   ...
   * index in the slice to the function f. The first expression is equivalent to: let fun ff (i,a,l)
   * = f(i,a)::l in Vector.fromList (rev (foldli ff [] sl)) end The latter expression is equivalent
   * to: mapi (f o #2) sl
   *)
   ...
   * indices), until f(x) evaluates to true; it returns true if such an x exists and false
   * otherwise.
   *)
