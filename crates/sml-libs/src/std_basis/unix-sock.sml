...
signature UNIX_SOCK (* OPTIONAL *) = sig
  ...
  structure Strm : sig
    (*!
     * This function creates a stream socket in the Unix address family. It raises SysErr if there
     * are too many sockets in use.
     *)
    val socket : unit -> 'mode stream_sock
    (*!
     * This function creates an unnamed pair of connected stream sockets in the Unix address family.
     * It is similar to the Posix.IO.pipe function in that the returned sockets are connected, but
     * unlike pipe, the sockets are bidirectional. It raises SysErr if there are too many sockets in
     * use.
     *)
  ...
  structure DGrm : sig
    ...
    (*!
     * This function creates an unnamed pair of connected datagram sockets in the Unix address
     * family. It raises SysErr if there are too many sockets in use.
     *)
