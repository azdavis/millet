...
signature MONO_VECTOR = sig
   ...
   * the function f. The expression app f vec is equivalent to: appi (f o #2) vec
   *)
   ...
   * argument vector. The more general mapi function supplies both the element and the element's
   * index to the function f. The expression mapi f vec is equivalent to: fromList (List.map f
   * (foldri (fn (i,a,l) => (i,a)::l) [] vec)) The expression map f vec is equivalent to: mapi (f o
   * #2) vec
   *)
   ...
   * implementations of the indexed versions. The expression foldl f is equivalent to: foldli (fn
   * (_, a, x) => f(a, x)) A similar relation holds between foldr and foldri.
   *)
   ...
   * increasing indices), until f x evaluates to true; it returns true if such an x exists and false
   * otherwise.
   *)
