...
i <= i + n <= |v|, where |v| is the length of the vector v. Slices provide a convenient notation for
specifying and operating on a contiguous subset of elements in a vector.
!*)
signature MONO_VECTOR_SLICE = sig
  ...
  length j, that is, it corresponds to vec[i..i+j-1]. It raises Subscript if i < 0 or j < 0 or |vec|
  ...
  have |vec| = |sl| and, for 0 <= i < |sl|, element i of vec is sub (sl, i).
  !*)
  ...
  in the slice to the function f. The latter expression is equivalent to: mapi (f o #2) sl
  !*)
