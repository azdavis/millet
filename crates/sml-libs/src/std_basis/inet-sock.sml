...
signature INET_SOCK (* OPTIONAL *) = sig
  ...
  (*!
   * The type-scheme of Internet-domain stream sockets; The type parameter 'mode can be instantiated
   * to either Socket.active or Socket.passive.
   *)
  ...
  (*!
   * toAddr (ia, i) converts an Internet address ia and a port number i into a socket address (in
   ...
   * unspecified. This function corresponds to the INADDR_ANY constant in the C Sockets API. The
   * values created by this function are used to bind a socket to a specific port.
   *)
  val any : int -> sock_addr
  structure UDP : sig
    (*!
     * This creates a datagram socket in the INet address family with the default protocol. It
     * raises SysErr if there are too many sockets in use.
     *)
    val socket : unit -> dgram_sock
    (*!
     * socket' prot creates a datagram socket in the INet address family with the protocol number
     * prot. The interpretation of prot is system dependent, but a value of 0 is equivalent to
     * socket(). It raises SysErr if there are too many sockets in use.
     *)
  ...
  structure TCP : sig
    ...
    (*!
     * socket' prot creates a stream socket in the INet address family with the protocol number
     * prot. The interpretation of prot is system dependent, but a value of 0 is equivalent to
     * socket().
     *)
     ...
     * connection at any time, thereby reducing small packet traffic on slower WANs. When set to
     * true, packets are sent as fast as possible. [[Refer to Stevens, p.316.]]
     *)
