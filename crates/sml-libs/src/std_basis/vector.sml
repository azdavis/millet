...
signature VECTOR = sig
  ...
  (*!
  These apply the function f to the elements of a vector in left to right order (i.e., in order of
  increasing indices). The more general appi function supplies both the element and the element's
  index to the function f. These are respectively equivalent to: List.app f (foldri (fn (i,a,l) =>
  (i,a)::l) [] vec) List.app f (foldr (fn (a,l) => a::l) [] vec)
  !*)
  ...
  vector. The more general form mapi supplies f with the vector index of an element along with the
  element. These are respectively equivalent to: fromList (List.map f (foldri (fn (i,a,l) =>
  (i,a)::l) [] vec)) fromList (List.map f (foldr (fn (a,l) => a::l) [] vec))
  !*)
  ...
  versions. The last two expressions are respectively equivalent to: foldli (fn (_, a, x) => f(a,
  x)) init vec foldri (fn (_, a, x) => f(a, x)) init vec
  !*)
