signature SOCKET (* OPTIONAL *) = sig
  ...
  (*!
   * bind (sock, sa) binds the address sa to the passive socket sock. This function raises SysErr
   * when the address sa is already in use, when sock is already bound to an address, or when sock
   * has been closed.
   *)
  val bind : ('af, 'sock_type) sock * 'af sock_addr -> unit
  (*!
   * listen (sock, n) creates a queue (of size n) for pending questions associated to the socket
   * sock. The size of queue is limited by the underlying system, but requesting a queue size larger
   * than the limit does not cause an error (a typical limit is 128, but older systems use a limit
   * of 5). This function raises the SysErr exception if sock has been closed.
   *)
   ...
   * socket sock. The socket must have been bound to an address via bind and enabled for listening
   * via listen. If a connection is present, accept returns a pair (s,sa) consisting of a new active
   * socket s with the same properties as sock and the address sa of the connecting entity. If no
   * pending connections are present on the queue then accept blocks until a connection is
   * requested. One can test for pending connection requests by using the select function to test
   * the socket for reading. This function raises the SysErr exception if sock has not been properly
   ...
   * where s is a new active socket with the same properties as sock and sa is the the address of
   * the connecting entity. If there are no pending connections, then this function returns NONE.
   * This function raises the SysErr exception if sock has not been properly bound and enabled, or
   * it sock has been closed.
   *)
   ...
   * blocking the caller (which is typically true for datagram sockets, but not stream sockets),
   * then true is returned. Otherwise, false is returned and the connection attempt is started; one
   * can test for the completion of the connection by testing the socket for writing using the
   * select function. This function will raise SysErr if it is called on a socket for which a
   * previous connection attempt has not yet been completed.
   *)
  val connectNB : ('af, 'sock_type) sock * 'af sock_addr -> bool
  (*!
   * close sock closes the connection to the socket sock. This function raises the SysErr exception
   * if the socket has already been closed.
   *)
  val close : ('af, 'sock_type) sock -> unit
  datatype shutdown_mode = NO_RECVS | NO_SENDS | NO_RECVS_OR_SENDS
  (*!
   * shutdown (sock, mode) shuts down all or part of a full-duplex connection on socket sock. If
   * mode is NO_RECVS, further receives will be disallowed. If mode is NO_SENDS, further sends will
   * be disallowed. If mode is NO_RECVS_OR_SENDS, further sends and receives will be disallowed.
   * This function raises the SysErr exception if the socket is not connected or has been closed.
   *)
   ...
   * underlying socket. Thus, the expression sameDesc(sockDesc sock, sockDesc sock) will always
   * return true for any socket sock.
   *)
  val sameDesc : sock_desc * sock_desc -> bool
  (*!
   * select {rds, wrs, exs, timeout} examines the sockets in rds, wrs, and exs to see if they are
   * ready for reading, writing, or have an exceptional condition pending, respectively. The calling
   * program is blocked until either one or more of the named sockets is ``ready '' or the specified
   * timeout expires (where a timeout of NONE never expires). The result of select is a record of
   * three lists of socket descriptors containing the ready sockets from the corresponding argument
   * lists. The order in which socket descriptors appear in the argument lists is preserved in the
   * result lists. A timeout is signified by a result of three empty lists. This function raises
   * SysErr if any of the argument sockets have been closed or if the timeout value is negative.
   * Note that one can test if a call to accept will block by using select to see if the socket is
   * ready to read. Similarly, one can use select to test if a call to connect will block by seeing
  ...
  (*!
   * ioDesc sock returns the I/O descriptor corresponding to socket sock. This descriptor can be
   * used to poll the socket via pollDesc and poll in the OS.IO structure. Using the polling
   * mechanism from OS.IO has the advantage that different kinds of I/O objects can be mixed, but
   * not all systems support polling on sockets this way. If an application is only polling sockets,
  ...
  (*!
   * These functions send the bytes in the slice slice on the active stream socket sock. They return
   * the number of bytes actually sent. If the don't_route flag is true, the data is sent bypassing
   * the normal routing mechanism of the protocol. If oob is true, the data is sent out-of-band,
   * that is, before any other data which may have been buffered. These functions raise SysErr if
  ...
  (*!
   * These functions are the nonblocking versions of sendVec, sendVec', sendArr, and sendArr'
   * (resp.). They have the same semantics as their blocking forms, with the exception that when the
   * operation can complete without blocking, then the result is wrapped in SOME and if the
   * operation would have to wait to send the data, then NONE is returned instead.
   *)
   ...
   * resulting vector is the number of bytes that were successfully received, which may be less than
   * n. If the connection has been closed at the other end (or if n is 0), then the empty vector
   * will be returned. In the second version, if peek is true, the data is received but not
   * discarded from the connection. If oob is true, the data is received out-of-band, that is,
   * before any other incoming data that may have been buffered. These functions raise SysErr if the
   * socket sock has been closed and they raise Size if n < 0 or n > Word8Vector.maxLen.
   *)
  ...
  (*!
   * These functions read data from the socket sock into the array slice slice. They return the
   * number of bytes actually received. If the connection has been closed at the other end or the
   * slice is empty, then 0 is returned. For recvArr', if peek is true, the data is received but not
   * discarded from the connection. If oob is true, the data is received out-of-band, that is,
   * before any other incoming data that may have been buffered. These functions raise SysErr if
   * sock has been closed.
   *)
  ...
  (*!
   * These functions are the nonblocking versions of recvVec, recvVec', recvArr, and recvArr'
   * (resp.). They have the same semantics as their blocking forms, with the exception that when the
   * operation can complete without blocking, then the result is wrapped in SOME and if the
  ...
  (*!
   * These functions send the message specified by the slice slice on the datagram socket sock to
   * the address sa. These functions raise SysErr if sock has been closed or if the socket has been
  ...
  (*!
   * These functions send the message specified by the slice slice on the datagram socket sock to
   * the address If the don't_route flag is true, the data is sent bypassing the normal routing
   * mechanism of the protocol. If oob is true, the data is sent out-of-band, that is, before any
   * other data which may have been buffered. These functions raise SysErr if sock has been closed
   * or if the socket has been connected to a different address than sa.
   *)
  ...
  (*!
   * These functions are the nonblocking versions of sendVecTo, sendVecTo', sendArrTo, and
   * sendArrTo' (resp.). They have the same semantics as their blocking forms, with the exception
   * that if the operation can complete without blocking, then the operation is performed and true
   * is returned. Otherwise, false is returned and the message is not sent.
   *)
   ...
   * where the vector vec is the received message, and sa is the socket address from the which the
   * data originated. If the message is larger than n, then data may be lost. In the second form, if
   * peek is true, the data is received but not discarded from the connection. If oob is true, the
   * data is received out-of-band, that is, before any other incoming data that may have been
   * buffered. These functions raise SysErr if sock has been closed; they raise Size if n < 0 or n >
   * Word8Vector.maxLen.
   *)
   ...
   * bytes actually received. If the connection has been closed at the other end or the slice is
   * empty, then 0 is returned. For recvArrFrom', if peek is true, the data is received but not
   * discarded from the connection. If oob is true, the data is received out-of-band, that is,
   * before any other incoming data that may have been buffered. These functions raise SysErr if
   * sock has been closed.
   *)
   ...
   * that when the operation can complete without blocking, then the result is wrapped in SOME and
   * if the operation would have to wait for input, then NONE is returned instead.
   *)
