 ...
 * arbitrarily large integers. Operations in IntInf that return a value of type IntInf.int should
 * never raise the Overflow exception. Note that, as it extends the INTEGER interface, IntInf
 * defines a type int. Any use of this type below, unmodified by a structure, refers to the local
 * type int defined in IntInf.
 *)
signature INT_INF (* OPTIONAL *) = sig
   ...
   * which pow(2, k) <= i. It raises Domain if i <= 0 and Overflow if the result is not
   * representable as an Int.int.
   *)
  val log2 : int -> Int.int
  (*!
   * These functions return the bit-wise OR, bit-wise exclusive OR, and bit-wise AND, respectively,
   * of the arguments.
   *)
  val orb : int * int -> int
   ...
   * n are interpreted as integers, the latter non-negative, this returns (i * 2(n)).
   *)
  val << : int * Word.word -> int
  (*!
   * ~>> (i, n) shifts i to the right by n bit positions. When i and n are interpreted as integers,
   * the latter non-negative, this returns floor(((i / 2(n)))).
   *)
  val ~>> : int * Word.word -> int
end
