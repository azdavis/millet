 ...
 * POSIX parlance, the password file and the group file). These are the data and operations
 * described in Section 9 of the POSIX standard 1003.1,1996[CITE].
 *)
signature POSIX_SYS_DB = sig
  ...
  structure Passwd : sig
     ...
     * directory, and the initial command shell, respectively, of the user corresponding to the
     * passwd value. The names of the corresponding fields in C are the same, but prefixed with
     * "pw_". The one exception is that C uses pw_dir for the home directory.
     *)
  ...
  (*!
   * These return the group or user database entry associated with the given group ID or name, or
   * user ID or name. It raises OS.SysErr if there is no group or user with the given ID or name.
   *)
