(*!
 * The structure Posix.IO specifies functions that provide the primitive POSIX input/output
 * operations, as described in Section 6 of the POSIX standard 1003.1,1996[CITE].
 *)
signature POSIX_IO = sig
  ...
  eqtype pid
  (*!
   * This creates a pipe (channel) and returns two file descriptors that refer to the read (infd)
   * and write (outfd) ends of the pipe.
   *)
  val pipe : unit -> {infd : file_desc, outfd : file_desc}
  (*!
   * dup fd returns a new file descriptor that refers to the same open file, with the same file
   * pointer and access mode, as fd. The underlying word (see Posix.FileSys.fdToWord) of the
   * returned file descriptor is the lowest one available. It is equivalent to dupfd {old=fd,
  ...
  (*!
   * readVec (fd, n) reads at most n bytes from the file referred to by fd. The size of the
   * resulting vector is the number of bytes that were successfully read, which may be less than n.
   * This function returns the empty vector if end-of-stream is detected (or if n is 0). It raises
   ...
   * although 0 is also returned if the slice is empty. This function will raise OS.SysErr if there
   * is some problem with the underlying system call (e.g., the file is closed).
   *)
  ...
  (*!
   * dupfd {old, base} returns a new file descriptor bound to old. The returned descriptor is
   * greater than or equal to the file descriptor base based on the underlying integer mapping
   * defined by Posix.FileSys.fdToWord and Posix.FileSys.wordToFD. It corresponds to the POSIX fcntl
   * function with the F_DUPFD command.
   *)
  ...
  (*!
   * getfl fd gets the file status flags for the open file descriptor fd and the access mode in
   * which the file was opened. It corresponds to the POSIX fcntl function with the F_GETFL command.
   *)
  val getfl : file_desc -> O.flags * open_mode
  (*!
   * setfl (fd, fl) sets the file status flags for the open file descriptor fd to fl. It corresponds
   * to the POSIX fcntl function with the F_SETFL command.
   *)
  val setfl : file_desc * O.flags -> unit
  (*!
   * lseek (fd, off, wh) sets the file offset for the open file descriptor fd to off if wh is
   * SEEK_SET; to its current value plus off bytes if wh is SEEK_CUR; or, to the size of the file
   * plus off bytes if wh is SEEK_END. Note that off may be negative.
   *)
  ...
  structure FLock : sig
     ...
     * whence and start parameters give the beginning file position as in lseek. The len value
     * provides the number of bytes to be locked. If the section starts at the beginning of the file
     * and len = 0, then the entire file is locked. Normally, pid will be NONE. This value is only
     * used in a flock returned by getlk.
     *)
  ...
  (*!
   * getlk (fd, fl) gets the first lock that blocks the lock description fl on the open file
  ...
  (*!
   * setlk (fd, fl) sets or clears a file segment lock according to the lock description fl on the
   * open file descriptor fd. An exception is raised immediately if a shared or exclusive lock
   * cannot be set. It corresponds to the POSIX fcntl function with the F_SETLK command.
   *)
  val setlk : file_desc * FLock.flock -> FLock.flock
  (*!
   * This is similar to the setlk function above except that setlkw waits on blocked locks until
   * they are released. It corresponds to the POSIX fcntl function with the F_SETLKW command.
   *)
   ...
   * construct an input stream. The functions are comparable to the POSIX function fdopen. The
   * argument fields have the following meanings: fd A file descriptor for a file opened for
   * reading. name The name associated with the file, used in error messages shown to the user.
   * initBlkMode False if the file is currently in non-blocking mode, i.e., if the flag O.nonblock
   ...
   * argument fields have the following meanings: fd A file descriptor for a file opened for
   * writing. name The name associated with the file, used in error messages shown to the user.
   * initBlkMode False if the file is currently in non-blocking mode, i.e., if the flag O.nonblock
   * is set in #1(getfl fd). appendMode True if the file is in append mode, i.e., if the flag
   * O.append is set in #1(getfl fd). chunkSize The recommended size of write operations for
   * efficient writing.
   *)
